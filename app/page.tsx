...
const SpaceStation = lazy(() => import('@/components/SpaceStation'));
...
function LandingPage() {
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-64px)] relative overflow-x-hidden selection:bg-primary/20 selection:text-primary">
      {/* Background with glowing interactions */}
      <MouseGlowBackground />

      {/* Hero Viewport Wrapper */}
      <div className="relative w-full min-h-[calc(100vh-64px)] flex flex-col justify-center overflow-hidden">
        {/* 新增：空间站 — 严格定位在左下角边缘，悬浮于背景深处 */}
        <div
          className="absolute z-[0] pointer-events-none select-none left-[-30%] bottom-[-15%] md:left-[-15%] md:bottom-[-10%] w-[80vmax] h-[80vmax] md:w-[60vmax] md:h-[60vmax]"
        >
        ...
        {/* COBE Globe Background — anchored bottom-right, semi-transparent */}
        <div
        ...
        {/* Hero Section */}
        <section className="flex flex-col items-center justify-center text-center max-w-5xl mx-auto px-6 space-y-8 pt-24 pb-24 relative z-10 w-full flex-grow">
          <FadeInUp delay={0.1}>
          ...
          <FadeInUp delay={2.0} yOffset={20} className="flex gap-4 pt-4">
            <MagneticButton
              ...
              className="px-10 py-4 bg-primary text-white font-bold text-base rounded-2xl transition-colors transition-transform flex items-center gap-3 shadow-xl hover:shadow-primary/30 hover:-translate-y-1 active:scale-95 z-20"
            >
          ...
          {/* Scroll indicator */}
          <motion.div
            className="absolute bottom-8 left-1/2 -translate-x-1/2 text-slate-400 flex flex-col items-center gap-2 z-20"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
