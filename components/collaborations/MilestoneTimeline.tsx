...
const statusConfig: Record<string, { label: string; badgeClass: string }> = {
    INCOMPLETE: { label: 'LOCKED', badgeClass: 'text-slate-400 dark:text-slate-500 bg-slate-100' },
    ...
    return (
        <div className="space-y-4">
            {milestones.map((ms, index) => {
                ...
                if (isCompleted) {
                    return (
                        <div key={ms.id} className="bg-white rounded-xl border border-slate-200 dark:border-slate-700/60 p-6 flex items-start gap-4 opacity-75 hover:opacity-100 transition-opacity">
                ...
                if (isActive) {
                    return (
                        <div key={ms.id} className="relative bg-white rounded-xl border-l-4 border-l-primary border-y border-r border-slate-200 dark:border-slate-700/60 p-6 shadow-[0_20px_40px_-12px_rgba(0,0,0,0.05)] hover:-translate-y-0.5 transition-transform">
                                ...
                                <div className="flex-1">
                                    ...
                                    {msProofs.length > 0 && (
                                        <div className="mt-4 space-y-2">
                                            {msProofs.map(proof => (
                                                <div key={proof.id} className="bg-[#F0F1F5]/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700/60 flex items-center gap-3">
                                    ...
                                    {isProvider && ms.status === 'INCOMPLETE' && (
                                        <div className="mt-6 bg-[#F0F1F5]/50 rounded-lg p-5 border border-dashed border-slate-300">
                                            <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-3">{t.quests.submitDeliverable}</label>
                                            <button
                ...
                // Locked milestone
                return (
                    <div key={ms.id} className="bg-[#F0F1F5]/50 rounded-xl border border-slate-200 dark:border-slate-700/60 p-6 flex items-start gap-4 opacity-60">
                        <div className="mt-1 flex-shrink-0">
                            <div className="w-6 h-6 rounded-full bg-slate-200 text-slate-400 dark:text-slate-500 flex items-center justify-center">
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 bg-slate-100 px-2 py-1 rounded">
                                    LOCKED
                                </span>
                            </div>
                            <p className="text-sm text-slate-400 dark:text-slate-500">{amount} USDC · {ms.amount_percentage}%</p>
                        </div>
                    </div>
                );
            })}
